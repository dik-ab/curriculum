...
[Prettier](prettier.html)と[ESLint](eslint.html)を導入しましたが、今のままでは「コマンドを打ったときだけ」動く状態です。人間はコマンドを打ち忘れる生き物なので、これでは品質を守りきれません。このページでは、**ファイルを保存した瞬間に自動で整形・修正される**ようVS Codeを設定し、さらにチームの誰もが同じチェックを同じコマンドで実行できるように**pnpm scripts**を整備します。
...
- **第3段（CI）**: 人間が忘れても、GitHubにプッシュされたコードをサーバー側が必ず検査します。これは[CI/CDの章](../cicd/ci_pipeline.html)で構築します。

このページでは第1段と第2段を整備します。

## VS Code拡張機能をインストールする

VS Codeに次の2つの拡張機能をインストールします。拡張機能のインストール手順自体は[拡張機能のセットアップ](../environment/live_preview.html)で学んだとおり、サイドバーのExtensionsアイコンから検索してInstallを押すだけです。
...
インストールが終わると、この時点でESLint拡張はすでに働き始めます。[前のページ](eslint.html)で試したような未使用変数を書くと、コマンドを実行しなくても**その場で赤い波線**が引かれ、マウスを乗せるとルール名付きの説明が表示されます。エディタ下部の「問題」パネル（表示メニュー → 問題）にも一覧表示されます。
...
スペースが詰められ、[`.prettierrc`](prettier.html) の設定どおりシングルクォートになり、セミコロンが付きました。これが**保存時フォーマット**です。もし整形されない場合は、次を確認してください。
...
エディタ連携（第1段）ができたので、次は共通コマンド（第2段）です。[これまでのページ](prettier.html)で `format` と `lint` は用意済みですが、ここで体系的に整理します。
...
そこで最後の砦が第3段のCI（Continuous Integration、継続的インテグレーション）です。[CI/CDの章のCIパイプライン構築](../cicd/ci_pipeline.html)では、GitHub Actionsを使って「プッシュやPull Requestのたびに、サーバー上で `pnpm run lint` などを自動実行し、失敗したらマージをブロックする」仕組みを作ります。そのとき実行されるのは、**まさにこのページで整備した pnpm scripts そのもの**です。スクリプト名を統一しておいたのは、CIの設定をシンプルに書けるようにするためでもあります。
...
- 前のページ: [リンタとESLint](eslint.html)
- 次の章: [バックエンドテスト](../testing/) — 「コードの動作が正しいか」を自動で検証するテストを学びます。lintが「書き方」の品質なら、テストは「振る舞い」の品質です。
- この章の続きは[CIパイプラインの構築](../cicd/ci_pipeline.html)で登場します。ここで整備した `lint` や `format:check` をGitHub Actionsに組み込み、プッシュのたびに自動実行させます。
- [SNS開発（最終プロジェクト）](../sns/project_setup.html)でも、プロジェクトの立ち上げ時にこの章の手順で開発環境を整えます。
