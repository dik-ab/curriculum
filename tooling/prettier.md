...
このページでは、コードの見た目を自動で整える**フォーマッタ（formatter、フォーマッタ）**について学び、JavaScript/TypeScript界隈で標準的に使われている**Prettier（プリティア）**を実際のプロジェクトに導入します。[章の概要](./)で見たとおり、フォーマッタは「コードの見た目」を担当するツールです。
...
- **差分が汚れる**: [Gitの章](../git/basic_commands.html)で学んだ `git diff` を思い出してください。誰かがエディタの設定でインデントを変えてしまうと、「中身は変わっていないのに何百行も差分が出る」コミットが生まれ、[Pull Requestのレビュー](../git/github_and_pr.html)が困難になります。
...
まずは小さなファイルで動きを体験しましょう。練習用のディレクトリを作って試します（pnpmの導入がまだの場合は[実践: フルスタックTodoアプリ](../fullstack-todo/setup.html)を参照してください）。
...
整形してほしくないファイルやディレクトリは、**`.prettierignore`** に列挙します。書き方は[Gitの章で学んだ](../git/basic_commands.html) `.gitignore` と同じです。
...
ここまで手作業で導入してきましたが、実は[バックエンドの章](../backend/setup.html)で `nest new` コマンドを実行したとき、**Nest CLIがPrettier一式を自動でセットアップしています**。NestJSプロジェクトのルートを見てみましょう。
...
一方、[Viteで作成したReactプロジェクト](../react/setup.html)はどうでしょうか。Viteのテンプレートには**ESLintは含まれていますが、Prettierは含まれていません**（ESLintについては[次のページ](eslint.html)で詳しく見ます）。そこで、Reactプロジェクトには自分でPrettierを追加します。
...
見た目はPrettierに任せられるようになりました。次の[リンタとESLint](eslint.html)では、コードの**中身の問題**（未使用変数や危険な書き方）を検出するリンタを学びます。そこでは「PrettierとESLintのルールが衝突する問題」とその解決方法も扱うので、このページの内容が前提になります。

また、このページで整備した `format` スクリプトは、[エディタ連携とpnpm scripts](editor_and_scripts.html)で「保存した瞬間に自動実行」へ進化させ、さらに[CI/CDの章](../cicd/ci_pipeline.html)ではプッシュのたびに `--check` で自動検査する仕組みに組み込みます。

- 前のページ: [コード品質と開発ツール（章の概要）](./)
- 次のページ: [リンタとESLint](eslint.html)
