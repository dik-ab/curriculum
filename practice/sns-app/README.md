...
カリキュラム「[SNS開発（最終プロジェクト）](https://dik-ab.github.io/curriculum/sns/)」の回答コードです。
React（Vite）+ NestJS + Prisma + PostgreSQL（+ Socket.IO / AWS SDK）で、認証つきのSNS（投稿・いいね・フォロー・DMチャット・プロフィール編集・画像アップロード）を実装しています。

対応する章:

- [プロジェクトセットアップ](https://dik-ab.github.io/curriculum/sns/project_setup.html) — `compose.yaml` / `backend/` / `frontend/` の骨格、ValidationPipe・CORS・PrismaModule・`GET /health`
- [ユーザー登録とログイン（JWT認証）](https://dik-ab.github.io/curriculum/sns/auth.html) — `backend/src/auth/`、`frontend/src/lib/apiClient.ts`、`useHashRoute`
- [メールアドレス確認（SES）](https://dik-ab.github.io/curriculum/sns/email_verification.html) — `backend/src/mail/`、`EmailVerificationToken`、`#/verify-email`
- [投稿機能とタイムライン](https://dik-ab.github.io/curriculum/sns/posts.html) — `backend/src/posts/`、`TimelinePage` / `PostCard` / `Layout`
- [いいね機能](https://dik-ab.github.io/curriculum/sns/likes.html) — `backend/src/likes/`（P2002→409 / P2025→404）
- [フォローとフォロー中タイムライン](https://dik-ab.github.io/curriculum/sns/follow.html) — `backend/src/users/`、`GET /posts/timeline`、`UserPage`
- [DMチャット（リアルタイム）](https://dik-ab.github.io/curriculum/sns/chat.html) — `backend/src/chat/`（REST + ChatGateway）、`ChatPage`
- [プロフィール編集と画像アップロード](https://dik-ab.github.io/curriculum/sns/profile_and_images.html) — `PATCH /users/me`、presigned URL発行、`SettingsPage`
- [SNSのテストを書く](https://dik-ab.github.io/curriculum/sns/testing.html) — `posts.service.spec.ts`（単体）、`test/follow.e2e-spec.ts`（E2E）
- [AWSへの全体デプロイ](https://dik-ab.github.io/curriculum/sns/deploy.html) — `backend/Dockerfile`、`infra/`（CDK）、`.github/workflows/`
...
- **SES（確認メールの実送信）** — `MAIL_TRANSPORT="ses"` に切り替えると `@aws-sdk/client-ses` で実送信しますが、送信元アドレスの検証などSES側の設定（[SESの章](https://dik-ab.github.io/curriculum/aws/ses.html)）が必要です。ローカル検証は `console` 輸送のみ実施しています。
...
- **デプロイ（`infra/` と `deploy.yml`）** — CDKスタックは `pnpm exec cdk synth` が通ることまで確認済みで、**実際のデプロイ（`cdk deploy`）は行っていません**。デプロイ手順・料金の注意は [deployの章](https://dik-ab.github.io/curriculum/sns/deploy.html) を参照してください（RDS / ALB / NAT Gateway は時間課金です。練習後は必ず `cdk destroy`）。
