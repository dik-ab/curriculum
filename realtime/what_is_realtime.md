...
[HTTPとREST](../backend/http_and_rest.html)で学んだとおり、HTTPは「クライアントが聞く → サーバーが答える」の繰り返しでした。まずは、このモデルのどこに限界があるのかから確認していきます。
...
ただし、ポーリングには「**仕組みが単純で、普通のHTTPだけで実現できる**」という大きな利点があります。実は[React基礎のfetchでAPI通信](../react/api_fetch.html)の知識だけで実装できます。`setInterval`でfetchを繰り返すだけだからです。「1分に1回更新されれば十分」というダッシュボードのような画面なら、今でもポーリングは合理的な選択です。
...
このカリキュラムの最終プロジェクトで作る[SNSのDMチャット](../sns/chat.html)は、まさに「お互いにメッセージを送り合う」機能なので、WebSocketを採用します。
...
- `fetchMemos()` — メモ一覧を取得して画面のリストを描き直す関数です。中身は[fetchでAPI通信](../react/api_fetch.html)で学んだことと同じです。
...
- **DMチャット** — 相手の発言が即座に届き、自分も送る双方向の機能なので、**WebSocket**を使います（[SNSのDMチャット](../sns/chat.html)で実装します）。
...
4つの方式の全体像がつかめたら、次は本命のWebSocketを深掘りします。[WebSocketの基礎](websocket_basics.html)では、ハンドシェイクの中身を確認し、実際にブラウザとNode.jsでWebSocketのエコーアプリを動かします。

ここで学んだ方式の比較は、最終プロジェクトの[SNSのDMチャット](../sns/chat.html)で「なぜWebSocketを選ぶのか」を判断する根拠になります。
