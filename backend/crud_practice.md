...
このページはセクションの集大成です。[HTTPとREST](http_and_rest.html)で設計し、[Controller](controller.html)・[Service](service_and_di.html)・[DTO](dto_and_validation.html)と部品を揃えてきたメモAPIを、ここで完成させます。作成・取得・更新・削除の4操作を一通り実装し、curlですべてのエンドポイントの動作を確認します。
...
これから作るメモAPIの全体仕様です。[HTTPとREST](http_and_rest.html)で設計した表を、実装に必要な情報まで含めて確定させます。
...
| `UnauthorizedException` | 401（[SNS開発の認証](../sns/auth.html)で使用） |
| `NotFoundException` | 404 |
| `InternalServerErrorException` | 500 |

なお、これらの例外を使わずに**予期しないエラー**（プログラムのバグなど）が起きた場合、NestJSは自動で500 Internal Server Errorを返します。[HTTPとREST](http_and_rest.html)で学んだ「500はサーバー側の誤り」が、フレームワークの既定動作として実現されているわけです。
...
- ParseIntPipeとDTO — [前ページ](dto_and_validation.html)で導入した検証が全ルートに効いています。不正なIDは400、不正なボディも400、存在しないIDは404（Serviceの例外）と、異常系の応答がすべて自動化されています。
...
また、今のAPIは**誰でも**メモを作成・削除できます。「自分のメモだけ操作できる」ようにするには認証（ログイン）と、それに基づくリクエストの保護（Guard）が必要です。これはSNS開発セクションの[認証](../sns/auth.html)で導入します。
...
認証（ユーザーが誰であるかを確認する仕組み。ログイン）と、認証情報に基づいてリクエストを通すか弾くかを判断するGuard（ガード）です。どちらもこのセクションでは扱わず、SNS開発セクションの[認証](../sns/auth.html)で初めて実装します。
...
おめでとうございます。設計（REST）から実装（NestJS）、検証（curl）まで、APIをゼロから完成させる一連の流れを経験しました。次の[練習問題](practice.html)で、このセクション全体の理解を確認しましょう。
