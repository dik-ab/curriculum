...
[HTTPとREST](http_and_rest.html)で、これから作るものが「リクエストを受け取ってJSONを返すAPIサーバー」だと分かりました。このページでは、それを作るための道具であるNestJS（ネストジェイエス）を紹介します。なぜフレームワークを使うのか、その中でなぜNestJSなのか、そしてNestJSアプリがどんな部品（Module / Controller / Service）で構成されるのかを、コードを書き始める前に俯瞰します。
...
- `constructor(private readonly memosService: MemosService)` — 「このクラスはMemosServiceを使います」という宣言です。インスタンスの生成はNestJSが自動で行います（この仕組みが[ServiceとDI](service_and_di.html)で学ぶ依存性注入です）。
...
なお、デコレータはTypeScriptの設定で有効化が必要な機能ですが、Nest CLIが生成するプロジェクトでは最初から有効になっているため、設定を意識する必要はありません（[環境構築とプロジェクト作成](setup.html)で確認します）。
...
|---|---|---|---|
| `@Module(...)` | クラス | この機能のまとまり（Module）を定義する | [ServiceとDI](service_and_di.html) |
| `@Controller(...)` | クラス | リクエストの受付係にする | [Controllerとルーティング](controller.html) |
| `@Get()` `@Post()` など | メソッド | メソッドとHTTPメソッド・パスを結びつける | [Controllerとルーティング](controller.html) |
| `@Param()` `@Query()` `@Body()` | 引数 | リクエストの各部分を引数として受け取る | [Controllerとルーティング](controller.html) |
| `@Injectable()` | クラス | DIで注入できる部品にする | [ServiceとDI](service_and_di.html) |
| `@IsString()` など | プロパティ | 入力値の検証ルールを宣言する | [DTOとバリデーション](dto_and_validation.html) |
...
1つのWebアプリは、この2つが[HTTPとREST](http_and_rest.html)で学んだ通信で会話することで成立します。どちらも「TypeScriptで、部品を組み合わせて作る」という思想は共通なので、Reactで身につけた感覚の多くがNestJSでも役立ちます。
...
NestJSには他にも多くの仕組みがありますが、このセクションでは上記の基本に絞ります。特に、リクエストを処理前に検査して通行を許可・拒否する**Guard（ガード）**という仕組みは、ログイン機能とセットで使うものなので、SNS開発セクションの[認証](../sns/auth.html)で初めて登場します。「そういうものが後で出てくる」とだけ覚えておいてください。
...
全体像がつかめたので、次の[環境構築とプロジェクト作成](setup.html)では、実際にNest CLIでプロジェクトを作成し、生成されたファイルを1つずつ読み解きながらサーバーを起動します。
