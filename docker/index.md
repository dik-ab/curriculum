...
| ページ | 内容 |
|---|---|
| [コンテナとは何か](what_is_container.html) | コンテナの概念、仮想マシン（VM）との違い、なぜ使われるのか |
| [Dockerのインストールと基本操作](install_and_basics.html) | Docker Desktopの導入、イメージとコンテナ、基本コマンド |
| [Dockerfileを書く](dockerfile.html) | NestJSのメモAPIをコンテナ化する。レイヤー構造とマルチステージビルド |
| [Docker Composeで複数コンテナを動かす](docker_compose.html) | compose.yamlの書き方、ボリューム、コンテナ間ネットワーク |
| [開発環境をComposeで組む](dev_environment.html) | PostgreSQL 16をComposeで起動し、ローカルのAPIとつなぐ開発環境の標準形を作る |
...
- [バックエンド基礎（NestJS）](../backend/)を修了し、メモAPI（[CRUD実践](../backend/crud_practice.html)で作成）が手元にあること
- [ターミナルの使い方](../environment/terminal.html)に慣れていること

それでは、[コンテナとは何か](what_is_container.html)から始めましょう。
