...
| `/init` | コードベースを分析してCLAUDE.mdを生成する（[前ページ](claude_md.html)参照） |
| `/memory` | 読み込まれているCLAUDE.md類の一覧を表示・編集する |
| `/permissions` | 許可ルールを確認・編集する（[導入ページ](claude_code_setup.html)参照） |
| `/model` | 使用するAIモデルを切り替える |
| `/plan` | プランモードに切り替えて計画から始める |
| `/resume` | 過去のセッションを選んで再開する |
| `/exit` | セッションを終了する |

`/clear` と `/compact` は、[LLMとは何か](what_is_llm.html)で学んだコンテキストウィンドウの管理コマンドです。会話が長くなるほどコンテキストは消費されていくので、別の話題に移るときは `/clear`、同じ作業を続けながら整理したいときは `/compact` を使います。
...
| `~/.claude/skills/`（後述のskills形式） | 自分の全プロジェクト |
...
[前のページ](claude_md.html)で「長い手順書をCLAUDE.mdに書くとコンテキストの無駄になる」と学びました。skillsはこの問題への答えです。
...
これで、Claude Codeを「導入し（[導入](claude_code_setup.html)）、プロジェクトに適応させ（[CLAUDE.md](claude_md.html)）、定型作業を部品化する（このページ）」ところまで揃いました。次のページ[AIペアプログラミング実践](ai_pair_programming.html)では、これらの道具を使って実際の開発をどう進めるか——計画→実装→レビューのループと、学習中の皆さんがAIに「奪われてはいけないもの」について学びます。
