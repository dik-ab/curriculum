...
このセクションの仕上げとして、AIと一緒に開発を進める**実践的なワークフロー**を学びます。道具の使い方はすでに揃いました。[LLMの仕組み](what_is_llm.html)を理解し、[Claude Codeを導入し](claude_code_setup.html)、[CLAUDE.md](claude_md.html)と[skills](skills_and_commands.html)でプロジェクトに適応させました。残る問いは「**で、実際の開発でどう使うのか？**」です。
...
いきなり「実装して」と頼まず、まず**planモード**（[導入ページ](claude_code_setup.html)参照。Shift+Tabで切り替え）で調査と計画をさせます。
...
計画段階での対話には、設計レビューと学習の両方の価値があります。「なぜその設計か」を説明させると、[データベースとPrisma](../database/relations.html)で学んだ多対多リレーションのような知識が実際の文脈とつながります。
...
実装が終わったら、必ず検証します。[LLMとは何か](what_is_llm.html)で学んだとおり、AIの出力の正しさは出力の外側でしか確認できません。
...
長い開発セッションでは、コンテキストウィンドウ（[LLMとは何か](what_is_llm.html)参照）が会話履歴で埋まっていきます。コンテキストが溢れそうになると古い内容が要約・圧縮されるため、初期に伝えた細かい指示が効かなくなることがあります。次の習慣で防ぎます。
...
- **会話で繰り返した指示はCLAUDE.mdへ**: `/clear` すると会話中の指示は消えます。消えて困る指示はCLAUDE.mdに書く、というのが[CLAUDE.mdのページ](claude_md.html)で学んだ「言い直しのサイン」の実践です
...
| 危険な操作をしようとした | 許可ルールのdenyに追加する（[導入ページ](claude_code_setup.html)参照） |
| 毎回同じ手順ミスをする | 正しい手順をskillにする（[skillsのページ](skills_and_commands.html)参照） |
...
次のセクション[AIチャット開発（RAG）](../ai-chat/)では、立場が変わります。AIを「使う側」から、**AIを組み込んだアプリケーションを「作る側」**になり、「このカリキュラムに質問できるQ&Aボット」を開発します。そこで直面するのが、このセクションで学んだコンテキストウィンドウの限界です。カリキュラム全体のような大量の知識はコンテキストに入りきりません。この問題を「必要な部分だけ検索してコンテキストに入れる」ことで解決する技術が **RAG（検索拡張生成）** です。まずは[RAGとは何か](../ai-chat/what_is_rag.html)から始めましょう。
