...
- `env: DATABASE_URL: ...` — その代わりに、各ステップへ環境変数として接続文字列を直接渡します。[Prismaのセットアップ](../database/prisma_setup.html)で `.env` に書いていたものを、CIではステップの `env` で渡す、という対応関係です。同じ構成は[SNS開発のテスト](../sns/testing.html)でも実際に組みます
