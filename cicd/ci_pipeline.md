...
ポイントは、backendとfrontendが**それぞれ独立したpnpmプロジェクト**（それぞれにpackage.jsonがある）だということです。NestJSプロジェクトの作り方は[NestJSのセットアップ](../backend/setup.html)、Reactプロジェクトの作り方は[Reactのセットアップ](../react/setup.html)で学んだとおりです。手元に両方そろったリポジトリがない場合は、それぞれのセットアップ手順で `backend/`・`frontend/` を作成してから進めてください。
...
|---|---|---|---|
| lint | `pnpm run lint` | コードスタイルの統一、典型的なバグパターンの検出 | [ESLint](../tooling/eslint.html) |
| test | `pnpm test` | コードが仕様どおりに動くこと | [単体テスト](../testing/unit_test.html) |
| build | `pnpm run build` | TypeScriptの型エラーがなく、実行可能な成果物が作れること | [コンパイルとは](../typescript/compile.html) |
...
3つとも成功すること（エラーなく終了すること）を確認してください。`pnpm run build` が成功すると `dist/` ディレクトリが生成されます。これが「ビルドの成果物」ですが、詳しくは[次のページ](build_and_deploy_flow.html)で扱います。
...
CIの真価はPull Requestで発揮されます。[GitHubとPull Request](../git/github_and_pr.html)で学んだ流れに、CIがどう加わるかを体験しましょう。
...
たとえば[単体テスト](../testing/unit_test.html)で書いたテストの期待値を、わざと間違った値に変更してcommit & pushし、PRを作成します。すると、PRのチェック欄は次のようになります。
...
[E2Eテスト](../testing/e2e_test.html)で書いたsupertestによるテストは、PostgreSQLのテストDBに接続して動くものでした。これをCIで回すには、ランナー上にPostgreSQLを用意する必要があります。GitHub Actionsには、そのための **サービスコンテナ（service container）** という仕組みがあります。ジョブの実行中だけ、指定したDockerイメージのコンテナを起動して使える機能です（コンテナとイメージについては[Docker基礎](../docker/)を参照してください）。
...
- `image: postgres:16` — 起動するDockerイメージです。[PostgreSQLのセットアップ](../database/postgresql_setup.html)でcomposeに書いたものと同じイメージ指定です
...
- `env: DATABASE_URL: ...` — テスト実行ステップに環境変数として接続文字列を渡します。[Prismaのセットアップ](../database/prisma_setup.html)で `.env` に書いていたものを、CIではステップの `env` で渡す、という対応関係です。E2Eの前にマイグレーション（`pnpm exec prisma migrate deploy` など）を実行するステップを挟む構成は、[SNS開発のテスト](../sns/testing.html)で実際に組みます
...
- `![CI](...)` — Markdownの画像記法です。URLの画像（バッジ）を表示します
...
ところで、CIの最後で実行している `pnpm run build` は、いったい**何を**作っているのでしょうか。次の[ビルドとデプロイの流れ](build_and_deploy_flow.html)では、この「ビルドの成果物」の正体を確かめ、それをユーザーに届けるCD（デプロイ自動化）の全体像へ進みます。
