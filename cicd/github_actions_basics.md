...
まず、CIらしいことは何もしない、メッセージを表示するだけの最小ワークフローを動かしてみます。[Git/GitHub基礎](../git/github_and_pr.html)で作成した練習用リポジトリ（なければ新しいリポジトリ）を使ってください。
...
- `node-version: '20'` — Node.js 20系をインストールします。このカリキュラムの開発環境（[Node.jsの導入](../environment/node.html)）と同じバージョンに揃えることで、「手元では動くがCIでは動かない」を防ぎます
...
`run` と `uses` が分かったので、実際のCIに近いワークフローを読んでみましょう。テスト（[Jest](../testing/unit_test.html)）が設定済みのNode.jsプロジェクトを想定します。
...
ワークフローの読み書きができるようになりました。次の[CIパイプラインを作る](ci_pipeline.html)では、この知識を使って、NestJSバックエンドとReactフロントエンドからなるリポジトリに lint + test + build のCIを組み込み、Pull Request上でチェックが緑になるところまでを実践します。

また、ここで読んだ `jekyll.yml` のような「ビルドして成果物をデプロイする」構成は、[ビルドとデプロイの流れ](build_and_deploy_flow.html)と[AWSデプロイ](../aws/deploy_from_cicd.html)につながっていきます。
