...
[前のページ](postgresql_setup.html)では、psqlから生のSQLでPostgreSQLを操作しました。このページでは、TypeScriptのコードからデータベースを操作するためのツール **Prisma（プリズマ）** を導入します。インストールから設定ファイルの読み解きまで、開発の土台を固めるページです。
...
このセクションでは、[バックエンド基礎で作ったメモAPI](../backend/crud_practice.html)のプロジェクトにPrismaを導入し、[CRUDのページ](crud_with_prisma.html)でメモをデータベースに永続化します。メモAPIのプロジェクト（`memo-api`）をVS Codeで開いてください。

また、[前のページ](postgresql_setup.html)の手順でPostgreSQLが起動していることを確認します。
...
メモAPIプロジェクトのルート（`package.json` のあるディレクトリ）で、pnpmを使って2つのパッケージをインストールします（pnpmの導入は[React基礎のセットアップ](../react/setup.html)を参照）。
...
この時点ではまだテーブルの定義（モデル）がありません。モデルは[次のページ](schema_and_migration.html)で書きます。
...
`.env` は**環境変数（environment variables、かんきょうへんすう）**を定義するファイルです。`prisma init` が生成した値はサンプルなので、自分の環境（[前のページ](postgresql_setup.html)のcompose.yaml）に合わせて書き換えます。
...
- `appdb` — `POSTGRES_DB` で作成したデータベース名です
...
1. **秘密情報をGitに含めないため** — 本番データベースのパスワードがGitHubに公開されたら大事故です。`.env` を [.gitignore](../git/basic_commands.html) に入れておけば、リポジトリに混入しません
2. **環境ごとに接続先を切り替えるため** — 開発中はlocalhost、本番では[RDS](../aws/rds.html)、と接続先が変わります。コードを変えずに `.env` だけ差し替えれば対応できます
...
[前のページ](postgresql_setup.html)で作った `users` / `posts` テーブルが残っていれば、次のように表示されます。
...
- `appdb` — `POSTGRES_DB: appdb` で自動作成されたデータベース名
...
- 前のページ: [PostgreSQLを起動して触ってみる](postgresql_setup.html)
- 次のページ: [スキーマ定義とマイグレーション](schema_and_migration.html)
- ここで設定した `.env` による接続情報の管理は、[AWSデプロイ](../aws/rds.html)で本番データベースに接続するときにも同じ考え方で登場します
