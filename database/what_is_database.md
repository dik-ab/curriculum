...
このページでは、データベースの基本概念を学びます。[バックエンド基礎](../backend/crud_practice.html)で作ったメモAPIは、データをメモリ上の配列に保存していたため、サーバーを止めるとデータが消えてしまいました。この問題を解決するのがデータベースです。まずは「データベースとは何か」「どういう仕組みでデータを管理しているのか」を、コードを書く前にしっかり理解しましょう。
...
[メモAPIの実装](../backend/crud_practice.html)では、次のようにデータを保存していました。
...
このように「ユーザー1人に対して投稿が複数ある」関係を**1対多（one-to-many）**と呼びます。リレーションの詳細は[リレーション](relations.html)のページで実際にコードを書きながら学びますが、「外部キーでテーブル同士をつなぐ」という基本イメージをここで持っておいてください。
...
SQLの基本は次の4つの操作です。これはHTTPメソッドやCRUDと綺麗に対応します（[HTTPとREST](../backend/http_and_rest.html)の復習です）。
...
それぞれの文法を、`users` テーブルを例に見てみましょう。実際に手を動かすのは[次のページ](postgresql_setup.html)で行うので、ここでは「読んで意味が分かる」ことを目指してください。
...
- アプリケーションとデータベースの間の言語は**SQL**です。ただし、TypeScriptのコードの中でSQLを文字列として組み立てるのは大変でミスも起きやすいため、実際の開発では**ORM**という道具を使ってTypeScriptの関数呼び出しでデータベースを操作します。これが[Prisma](prisma_setup.html)です
...
- 次のページ: [PostgreSQLを起動して触ってみる](postgresql_setup.html) — Docker ComposeでPostgreSQL 16を起動し、psqlで今日学んだSQLを実際に実行します
- このページで学んだ主キー・外部キー・1対多の関係は、[リレーション](relations.html)でPrismaのコードとして書き、[SNS開発](../sns/)のER図設計でフル活用します
