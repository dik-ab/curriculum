...
このセクションの総仕上げです。[Prisma ClientでCRUD](crud_with_prisma.html)でデータベース永続化したメモAPIを、自分の力でさらに育てていきます。スキーマの変更、マイグレーション、検索クエリ、そしてリレーションまで、セクション全体の知識を総動員する構成になっています。
...
- PostgreSQL 16がDocker Composeで起動している（[PostgreSQLを起動して触ってみる](postgresql_setup.html)）
- メモAPIがPrismaでデータベース永続化されている（[Prisma ClientでCRUD](crud_with_prisma.html)）
- `Memo` モデルに `done Boolean @default(false)` まで追加済み（[スキーマ定義とマイグレーション](schema_and_migration.html)）
...
- スキーマ変更→ `pnpm exec prisma migrate dev` の流れは[スキーマ定義とマイグレーション](schema_and_migration.html)の「2回目のマイグレーション」と同じです
- 既存データのあるテーブルにNOT NULLの列を足すとき、何が必要だったかを思い出してください
- DTOのバリデーションには `class-validator` の `@IsInt()` `@Min(0)` `@IsOptional()` が使えます（[DTOとバリデーション](../backend/dto_and_validation.html)）
...
- クエリパラメータの受け取りは `@Query('keyword')` です（[コントローラ](../backend/controller.html)）
...
メモを「仕事」「プライベート」のような**カテゴリ**で分類できるようにしてください。[リレーション](relations.html)で学んだ1対多の応用です。
...
カテゴリ自体を作るAPIも必要です。最小構成の例を載せます（[NestJSのモジュール構成](../backend/service_and_di.html)に従い、categoriesディレクトリを作ります）。
...
最後はチャレンジ問題です。1つのメモに複数の**タグ**を付けられるようにしてください。カテゴリと違い、メモから見てもタグは複数——つまり**多対多**です。[リレーション](relations.html)のLikeと同じ構造が使えます。
...
- 前のページ: [リレーション](relations.html)
- 次のセクション: [コード品質と開発ツール](../tooling/) — チーム開発に欠かせないPrettier/ESLintを導入します
- ここで身につけたスキーマ設計とクエリの力は、[バックエンドテスト](../testing/)でのテストDB、[RAG開発](../ai-chat/embeddings_and_pgvector.html)でのpgvector、そして[SNS開発](../sns/)でのユーザー・投稿・いいね・フォローの実装で、繰り返しフル活用します
