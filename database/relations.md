...
[前のページ](crud_with_prisma.html)まではテーブル1つ（Memo）だけを扱ってきました。しかし実際のアプリでは、複数のテーブルが関係し合います。このページでは、[データベースとは](what_is_database.html)で概念だけ学んだ**主キーと外部キーによる関係**を、Prismaの**リレーション（relation）**として定義し、クエリする方法を学びます。
...
- 重要な区別: `authorId` は**実際の列**ですが、`author` と `posts` は**テーブルには存在しません**。これらはPrismaがクエリ時にJOINなどで解決してくれる「仮想的なつながり」です。テーブルにあるのはあくまで外部キー `authorId` だけ、という点は[SQLで体験したposts.user_id](postgresql_setup.html)と同じです
...
マイグレーションを実行してテーブルを作ります（[前々ページ](schema_and_migration.html)の復習です）。
...
「投稿の一覧を、**投稿者の名前つきで**取得したい」——SQLでは[JOIN](postgresql_setup.html)を書いた場面です。Prismaでは `include` を使います。
...
SQLで書いた「JOINしてWHERE」（[JOIN入門](postgresql_setup.html)の最後の例）と同じことが、オブジェクトのネストで表現できています。
...
- 前のページ: [Prisma ClientでCRUD](crud_with_prisma.html)
- 次のページ: [練習問題](practice.html)
- このページのUser / Post / Likeは、[SNS開発](../sns/)でほぼそのままの形で再登場します。投稿機能は[投稿のCRUD](../sns/posts.html)で、いいね機能は[いいね](../sns/likes.html)で、さらにフォロー機能では「ユーザー対ユーザー」という自己参照の多対多（[フォロー](../sns/follow.html)）に発展します
