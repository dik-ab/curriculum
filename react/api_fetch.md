...
（実際にはもっと多くの項目がありますが、ここでは抜粋しています。）

この形式を**JSON（ジェイソン、JavaScript Object Notation）**と呼びます。名前のとおりJavaScriptのオブジェクトによく似た、データ交換用のテキスト形式です。[Reactとは何か](what_is_react.html)で「SPAはサーバーとデータだけをやり取りする」と学びましたが、その「データ」の標準形式がJSONです。
...
2. **データが届いたらstateを更新する**。すると[propsとstate](props_and_state.html)で学んだ仕組みにより、画面が自動で再描画されます。通信とReactをつなぐ接点は「set関数を呼ぶこと」だけです
...
「画面を開いたら1回だけデータを取得する」——これは[フック（useEffect）](hooks.html)で学んだ、**依存配列 `[]` のuseEffect**の出番です。
...
- state設計 — 「データ（`users`）」「読み込み中か（`isLoading`、初期値は `true`）」「エラーメッセージ（`error`、なければ `null`）」の3つで、通信の全状態を表現します。`string | null` は[ユニオン型](../typescript/basic_types.html)の実践です
...
- 早期リターン — [フォームとリスト](forms_and_lists.html)で学んだパターンです。「読み込み中なら」「エラーなら」を先に返し、本体のJSXは成功時だけに集中させます
...
- `method: "POST"` — HTTPメソッドの指定です。省略時はGET（取得）で、POSTは「新規作成」を意味します。メソッドの使い分けは[バックエンド基礎のHTTPとREST](../backend/http_and_rest.html)で体系的に学びます
...
サーバーからデータを取得し、ローディングとエラーを適切に扱って表示する——フロントエンドエンジニアの日常業務の核心を習得しました。次のページ[練習問題](practice.html)で、このセクション全体の総仕上げとして、入門編のTodoアプリをReactで作り直します。
