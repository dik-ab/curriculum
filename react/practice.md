...
[開発環境の構築](setup.html)の手順で、練習用のプロジェクトを新しく作ります。
...
- props型を `type` で定義することで、渡し忘れや型違いをエディタが検出してくれます（[propsとstate](props_and_state.html)）
- 「あるときだけ表示」は `&&` の出番です。`isStudent` はbooleanなので、そのまま左辺に置けます（[フォームとリスト](forms_and_lists.html)）

</details>

### 問題2：偶数カウンター（state）

[propsとstate](props_and_state.html)で作ったカウンターを、次の仕様で拡張してください。
...
いよいよ本番です。[入門編最終問題](../final_project.html)で作ったTodoアプリを、Reactで作り直します。
...
- `key={task.id}` — 削除や並びの変化があるリストなので、必ずIDをkeyにします。インデックスは不可です（[フォームとリスト](forms_and_lists.html)）
...
この対比を自分の言葉で説明できれば、[Reactとは何か](what_is_react.html)で学んだ「宣言的UI」を、知識ではなく経験として理解できたことになります。
...
- `useState<Filter>("all")` — ユニオン型のおかげで、`setFilter("done")` のような誤りはコンパイルエラーになります（[基本型](../typescript/basic_types.html)のユニオン型の実践です）

</details>

### 発展3：APIから初期データを読み込む

[fetchでAPI通信](api_fetch.html)の内容との総合問題です。マウント時に、JSONPlaceholderのTodo API（最初の5件）を初期タスクとして読み込んでください。
...
- ローディング・エラー・成功の3状態管理と早期リターンは、[fetchでAPI通信](api_fetch.html)で学んだ型そのままです
...
[フック（useEffect）](hooks.html)の最後で紹介したカスタムフックに挑戦します。発展3で書いた「取得・isLoading・error」のロジックを `useTodos` というカスタムフックに切り出し、`TodoApp` 本体を見た目の記述に集中させてください。
...
次のセクションは[バックエンド基礎（NestJS）](../backend/)です。[fetchでAPI通信](api_fetch.html)と発展課題3で「使う側」として触れたAPIを、今度は**自分で作ります**。バックエンド章を終えたら、ぜひこのTodoアプリに戻ってきて、JSONPlaceholderのURLを自作APIに差し替えてみてください。フロントエンドとバックエンドが自分のコードでつながる瞬間が、[SNS開発](../sns/)への第一歩です。
