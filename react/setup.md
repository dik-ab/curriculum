...
「TypeScriptはそのままではブラウザで動かないので、JavaScriptに変換する必要がある」という話を[コンパイル](../typescript/compile.html)で学びました。Viteはこの変換も自動で行ってくれるため、私たちは `tsc` を手で実行することなく、`.tsx` ファイルを保存するだけで結果を確認できます。
...
Viteの実行にはNode.jsが必要です。[Node.jsの導入](../environment/node.html)でインストール済みのはずなので、バージョンを確認しましょう。
...
`v20.x.x`（20系）であれば問題ありません。表示されない場合やバージョンが古い場合は、[Node.jsの導入](../environment/node.html)に戻ってセットアップし直してください。

## pnpmの導入

プロジェクトを作る前に、**pnpm（ピーエヌピーエム）**というツールを導入します。pnpmは、入門編の[Node.jsのインストール](../environment/node.html)で触れた**npm**と同じ役割を持つ**パッケージマネージャ**（パッケージのインストールやスクリプトの実行を行うツール）です。使い方はnpmとほとんど同じですが、一度ダウンロードしたパッケージをディスク上の1箇所にまとめて保存し、各プロジェクトからはそれを参照する仕組みになっているため、**ディスクの使用量が少なく、インストールも速い**という利点があります。
...
- `function App() { ... }` — `App` は**関数コンポーネント**です。「関数が画面の構造を返す」というReactの基本形で、詳細は[JSXとコンポーネント](jsx_and_components.html)で学びます
- `return (...)` — HTMLに似た記法（JSX）で画面の構造を返しています
- `useState(0)` — 画面の状態（ここではカウント）を管理する仕組みです。[propsとstate](props_and_state.html)で学びます
...
- **`tsconfig.json` / `tsconfig.app.json`** — [コンパイル](../typescript/compile.html)で学んだTypeScript設定です。テンプレートが適切に設定済みなので、そのまま使います
...
開発環境が整い、Reactアプリが手元で動くようになりました。次のページ[JSXとコンポーネント](jsx_and_components.html)では、`App.tsx` の中に書かれていたHTMLのような記法——**JSX**——のルールを学び、自分の手で画面の部品（コンポーネント）を作ります。
