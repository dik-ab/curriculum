...
このページから、実際に手を動かす量が増えます。[開発環境の構築](setup.html)で作った `my-react-app` で開発サーバー（`pnpm run dev`）を起動した状態で読み進めてください。
...
- `export default App;` — `main.tsx` がこの関数を `import` して描画します（[開発環境の構築](setup.html)で確認した流れです）
...
| `for="..."` | `htmlFor="..."` |
| `onclick="..."` | `onClick={...}` |
| `style="color: red"` | {% raw %}`style={{ color: "red" }}`{% endraw %} |
...
一方、`if` 文や `for` 文は「式」ではなく「文」なので、`{ }` の中には書けません。条件によって表示を変える方法は[フォームとリスト](forms_and_lists.html)で学びます。
...
条件分岐や繰り返しをJSXで表現する方法（三項演算子、`&&`、`map`）は[フォームとリスト](forms_and_lists.html)で学びます。
...
次のページ[propsとstate](props_and_state.html)では、コンポーネントに**外から値を渡す仕組み（props）**と、**コンポーネント自身が値を持って画面を更新する仕組み（state）**を学びます。[Reactとは何か](what_is_react.html)で予告した「データが変わると画面が変わる」体験は、次のページで実現します。ここで作った `components/` ディレクトリの構成は、[SNS開発](../sns/)まで一貫して使う整理方法です。
