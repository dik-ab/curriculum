...
このページでは、[セットアップ](setup.html)で作成した `backend/` プロジェクトに、[セクションの概要](./)で設計した5つのエンドポイントを実装します。NestJSのModule / Controller / Serviceの構成、DTOによるバリデーション、PrismaによるDBアクセスと、[バックエンド基礎](../backend/)・[データベースとPrisma](../database/)で学んだことをすべて組み合わせます。

実装の流れは、[メモAPIを作る](../backend/crud_practice.html)で経験したCRUD実装とほぼ同じです。違いは、データの保存先がメモリ上の配列ではなく**PostgreSQL**になることと、最初に設計したステータスコード（201 / 204 / 400 / 404）を意識して作り込むことです。
...
これから作るファイルと、リクエストが処理される流れを確認します。構造は[NestJSのアーキテクチャ](../backend/what_is_nestjs.html)で学んだModule / Controller / Serviceの3点セットに、Prisma用のModule / Serviceが加わった形です。
...
- **TodosController** — URLとHTTPメソッドを受け取り、適切なServiceのメソッドを呼びます。HTTPの世界とTypeScriptの世界の境界です（→ [Controller](../backend/controller.html)）
- **TodosService** — 「Todoをどう扱うか」というロジックの本体です（→ [ServiceとDI](../backend/service_and_di.html)）
- **PrismaService** — Prisma Clientをラップし、DIで配れるようにしたものです（→ [NestJSへの組み込み](../database/crud_with_prisma.html)）
...
まず、NestJSの中でPrisma Clientを使えるようにします。[Prisma ClientでCRUD](../database/crud_with_prisma.html)で学んだとおり、Prisma Clientを継承した**PrismaService**を作り、Module経由で配るのが定石です。

`backend/` ディレクトリで、[Nest CLI](../backend/setup.html)を使ってファイルを生成します。CLIはプロジェクトの開発依存に含まれているので、`pnpm exec`（プロジェクト内のコマンドを実行する）で呼び出せます。
...
- `@Injectable()` — このクラスをDI（依存性注入）の対象にします（→ [ServiceとDI](../backend/service_and_di.html)）
...
- `exports: [PrismaService]` — **他のModuleからもPrismaServiceを使えるように公開**します。これがないと、後で作るTodosModuleから注入できません（→ [Module](../backend/service_and_di.html)）
...
リクエストボディの形を定義し、不正な入力を弾きます。[DTOとバリデーション](../backend/dto_and_validation.html)で学んだ、class-validatorとValidationPipeの組み合わせです。
...
- `@MaxLength(100)` — 100文字以内に制限します。[スキーマ定義](setup.html)で `@db.VarChar(100)` とした、**DB側の制約と揃えている**点が重要です。DTOで弾かなければ、DBに入らない長さの文字列がエラーを引き起こします
...
DTOにデコレータを書いただけでは検証は動きません。[ValidationPipe](../backend/dto_and_validation.html)をアプリ全体に適用します。
...
ロジックの本体です。[Prisma ClientでCRUD](../database/crud_with_prisma.html)で学んだメソッド（`findMany` / `findUnique` / `create` / `update` / `delete`）を使います。
...
- `constructor(private readonly prisma: PrismaService)` — コンストラクタの引数に型を書くだけで、NestJSがPrismaServiceのインスタンスを**注入**してくれます（→ [DIの仕組み](../backend/service_and_di.html)）
...
HTTPの入口です。[セクションの概要](./)のAPI設計表と1行ずつ対応させながら書きます。
...
- `@Controller('todos')` — このControllerのURLの共通部分（プレフィックス）です。`@Get()` は `GET /todos`、`@Get(':id')` は `GET /todos/:id` に対応します（→ [ルーティング](../backend/controller.html)）
...
確認には[HTTPとREST](../backend/http_and_rest.html)でも使ったcurlを使います。`-i` はレスポンスヘッダー（ステータスコード）も表示するオプションです。
...
ModuleとDIの仕組みは[ServiceとDI](../backend/service_and_di.html)を参照してください。
...
APIが完成しました。次の[フロントエンド: 画面の実装](frontend.html)では、Reactでこの APIを呼び出す画面を作ります。`fetch` によるAPI通信は[fetchでAPI通信](../react/api_fetch.html)の復習です。

なお、このページで書いたService（存在確認 + 404、DTOによる検証）のパターンは、[SNS開発の投稿機能](../sns/posts.html)でもそのまま登場します。
