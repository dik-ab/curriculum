...
つまり、**「データベースだけコンテナ、アプリはローカル」**という形です。[開発環境をcomposeで組む](../docker/dev_environment.html)ではAPIもコンテナに入れる構成を学びましたが、開発中はコードの変更を高速に反映したいため、アプリはローカルの `pnpm run dev` で動かすほうが快適です。アプリのコンテナ化は、本番環境へのデプロイ（[AWSデプロイ](../aws/)）のための技術として位置づけます。
