...
このページでは、Todoアプリの開発を始めるための土台を作ります。具体的には、パッケージマネージャ**pnpm**の導入、リポジトリ構成の作成、Docker ComposeによるPostgreSQLの起動、NestJSプロジェクトとReactプロジェクトの作成、Prismaの初期化までを行います。

ここで作る構成は[セクションの概要](./)で説明した**開発環境の標準形**（DBはコンテナ、アプリはローカルで `pnpm run dev`）です。1つ1つの手順は過去の章で学んだことの組み合わせなので、迷ったら参照リンク先に戻って確認してください。
...
これまでパッケージの管理には `npm` を使ってきました。このセクションからは、**pnpm（ピーエヌピーエム）**というパッケージマネージャに切り替えます。
...
pnpmの導入には、[Node.js](../environment/node.html)（20系）に同梱されている**Corepack（コアパック）**という機能を使います。Corepackは「Node.jsに付属する、パッケージマネージャの管理ツール」で、これを有効化するだけでpnpmが使えるようになります。別途インストーラをダウンロードする必要はありません。
...
作業ディレクトリを作成し、[Git](../git/basic_commands.html)で管理を開始します。
...
データベースから準備します。[開発環境をcomposeで組む](../docker/dev_environment.html)・[PostgreSQLを起動して触ってみる](../database/postgresql_setup.html)で学んだ構成とほぼ同じです。
...
- `image: postgres:16` — PostgreSQL 16の公式イメージを使います。バージョンを固定して環境を再現可能にします（→ [Dockerfileの書き方](../docker/dockerfile.html)）
...
- `volumes: db-data:...` — [ボリューム](../docker/docker_compose.html)でデータを永続化し、コンテナを作り直してもTodoのデータが消えないようにします
...
fullstack-todo-db-1     postgres:16   "docker-entrypoint.s…"   db        15 seconds ago   Up 14 seconds   0.0.0.0:5432->5432/tcp
```

`STATUS` が `Up` になっていれば成功です。これでデータベースの準備は完了です。**今後、開発を始めるときは最初にこの `docker compose up -d` を実行する**、と覚えてください。

なお、[データベースとPrismaの章](../database/postgresql_setup.html)などで別のPostgreSQLコンテナを起動したままの場合、5432番ポートが衝突して起動に失敗します。`docker ps` で確認し、不要なコンテナは `docker compose down`（そのプロジェクトのディレクトリで実行）で停止してください（→ [Dockerの基本操作](../docker/install_and_basics.html)）。

## バックエンド（NestJS）プロジェクトを作る

`fullstack-todo/` 直下で、[Nest CLI](../backend/setup.html)を使ってプロジェクトを作成します。Nest CLIは `pnpm dlx`（npxに相当する、パッケージの一時実行コマンド）で呼び出します。
...
起動確認をします。NestJSの開発時は、ファイルの変更を監視して自動で再起動する**watchモード**（`start:dev`）を使います（→ [NestJSのセットアップ](../backend/setup.html)）。
...
次に、NestJSからPostgreSQLへ接続するためのPrismaをセットアップします。手順は[Prismaの導入](../database/prisma_setup.html)で学んだとおりです。
...
- `pnpm add -D prisma` — Prisma CLI（マイグレーションなどの開発用ツール）を開発依存として追加します
- `pnpm add @prisma/client` — アプリの実行時にDBへアクセスするためのライブラリを追加します
- `pnpm dlx prisma init` — `prisma/schema.prisma` と `.env` の雛形を生成します
...
`.env` にはパスワードなどの秘密情報が入るため、**Gitにコミットしてはいけません**（→ [Prismaの導入](../database/prisma_setup.html)）。`fullstack-todo/` 直下に `.gitignore` を作成しておきます。
...
[セクションの概要](./)のER図のとおり、Todoモデルを定義します。書き方は[モデル定義とマイグレーション](../database/schema_and_migration.html)の復習です。
...
マイグレーションを実行して、このモデルを実際のテーブルとしてDBに反映します（→ [マイグレーションとは](../database/schema_and_migration.html)）。
...
`migrate dev` は「マイグレーションファイルの生成」「DBへの適用」「Prisma Clientの生成」を一度に行うのでした。本当にテーブルができたか、psqlで覗いて確認してみましょう（→ [psqlでの確認](../database/postgresql_setup.html)）。
...
最後にフロントエンドです。`fullstack-todo/` 直下に戻り、[Viteでのプロジェクト作成](../react/setup.html)と同じ手順をpnpmで行います。
...
開発時の起動手順は次の3つです。この形が**開発環境の標準形**であり、[SNS開発](../sns/project_setup.html)でもそのまま使います。
...
ターミナルを3つ（DBは起動したら閉じてよいので実質2つ）並べて使うことになります。VS Codeのターミナル分割機能（→ [ターミナルの使い方](../environment/terminal.html)）を使うと便利です。

最後に、ここまでの成果をコミットしておきましょう（→ [基本コマンド](../git/basic_commands.html)）。
...
`pnpm dlx` は「パッケージをインストールせず、その場で一時的に実行する」コマンドで、npmの `npx` に相当します。なお、すでにプロジェクトの依存に入っているパッケージのコマンドを実行するだけなら `pnpm exec prisma ...` も使えます。
...
**Q4. `pnpm dlx prisma migrate dev --name init` は何を行うコマンドですか。3つ挙げてください。**
...
詳細は[モデル定義とマイグレーション](../database/schema_and_migration.html)を参照してください。
...
土台が完成しました。次の[バックエンド: Todo APIの実装](backend.html)では、NestJSとPrismaを使って[セクションの概要](./)で設計した5つのエンドポイントを実装し、curlで動作確認します。

なお、このページで確立した「リポジトリ構成 + DB起動 + Prisma初期化」の手順は、[SNS開発のプロジェクトセットアップ](../sns/project_setup.html)でもほぼ同じ流れで繰り返します。手順の意味を理解しながら進めておくと、その時に迷わず進められます。
