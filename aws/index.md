...
図の登場人物を簡単に紹介します（詳しくは[主要サービスの全体像](core_services.html)で1つずつ解説します）。
...
| ページ | 内容 |
|---|---|
| [AWSとは何か](what_is_aws.html) | クラウドの概念、リージョン/AZ、アカウント作成、IAM、**料金の注意と予算アラート** |
| [主要サービスの全体像](core_services.html) | VPC / S3 / CloudFront / ECR / ECS / ALB / RDS / SES / IAM / Route 53 を役割マッピングつきで解説 |
| [IaCとは何か](what_is_iac.html) | なぜ手作業ではだめか、CDKとTerraformの比較、本カリキュラムがCDKを採用する理由 |
| [CDK入門](cdk_setup.html) | CDK導入、スタック/コンストラクトの概念、最初のスタック、diff/deploy/destroy |
| [S3 + CloudFront](s3_cloudfront.html) | フロントエンドのデプロイ（CDKで構築 + Terraform対訳例） |
| [ECR + ECS Fargate](ecr_ecs.html) | APIのデプロイ（CDKで構築 + Terraform対訳例） |
| [RDS](rds.html) | 本番データベース（CDKで構築 + Terraform対訳例） |
| [SESでメール送信](ses.html) | サンドボックス、アドレス検証、NestJSからの送信実装 |
| [CI/CDから自動デプロイ](deploy_from_cicd.html) | GitHub Actions OIDCでキーレス認証、フロント/API双方の自動デプロイ |

## 前提知識

このセクションは中級編のこれまでの内容を総動員します。特に次のページの内容を前提にします。

- [Dockerfileの書き方](../docker/dockerfile.html) — APIをコンテナ化してECSで動かします
- [PostgreSQLのセットアップ](../database/postgresql_setup.html) — 本番ではRDSに置き換えます
- [ビルドとデプロイの全体像](../cicd/build_and_deploy_flow.html) — 「ビルド成果物をどこに置くか」の答えがこのセクションです
- [GitHub Actions基礎](../cicd/github_actions_basics.html) — 最終ページで自動デプロイに使います
...
>
> また、最初のページ[AWSとは何か](what_is_aws.html)で**予算アラート（AWS Budgets）の設定**を必ず行ってください。万一消し忘れても、メールで気づける保険になります。
...
それでは、[AWSとは何か](what_is_aws.html)から始めましょう。
